...
def solve_maze(maze, start, end):
    # Create a stack for DFS (depth first search)
    stack = []
...
def draw_path(x, y):
    canvas.delete("cat")
    canvas.create_image(y * cell_size + cell_size // 2, x * cell_size + cell_size // 2, image=cat, tags="cat")
    window.update()
...
# Set up the window
window = tk.Tk()
window.title("Gangester Amol XD")
cell_size = 50
